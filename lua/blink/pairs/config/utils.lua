...
--- @param schema table Schema for validating against
--- @param source table The original table that we're validating against
--- @see vim.validate
function utils.validate(path, schema, source)
  local _, err = pcall(utils._validate, schema)
  if err ~= nil then
    -- remove stack trace from error message
    err = table.concat(vim.list_slice(vim.split(err, ':'), 3), ':'):gsub('^%s+', '')
    error(path .. '.' .. err)
  end
  ...
  for k, _ in pairs(source) do
    if schema[k] == nil then
      ...
      require('blink.pairs.utils').notify(msg)
    end
  end
end
